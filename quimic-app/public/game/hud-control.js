function createHUD() {
    ...
    if (localHighScore !== null) {
        this.highScoreText.setText('MAXIMA PUNTUACIÓN\n' + localHighScore.toString().padStart(6, '0'))
    }
...
function gameOverScreen(outOfTime=false) {
    if (localStorage.getItem('high-score') !== null) {
        if (localStorage.getItem('high-score') < score) {
            ...
            this.highScoreText.setText('NUEVO RECORD!\n' + score.toString().padStart(6, '0'))
        }
    } else {
        localStorage.setItem('high-score', score);
    }
    ...
    this.add.bitmapText(screenCenterX, screenHeight / 3, 'carrier_command', outOfTime ? 'TIEMPO FUERA' : 'PERDISTE', screenWidth / 30).setOrigin(0.5).depth = 5;
    ...
    this.add.bitmapText(screenCenterX, screenHeight / 1.7, 'carrier_command', '> CAPTURA DE PANTALLA', screenWidth / 50).setOrigin(0.5).setInteractive().on('pointerdown', () => getScreenshot()).depth = 5;
}
